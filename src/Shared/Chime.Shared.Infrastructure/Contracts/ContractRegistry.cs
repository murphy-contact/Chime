...
namespace Chime.Shared.Infrastructure.Contracts;
...
internal sealed class ContractRegistry : IContractRegistry
{
    ...
    private static Type GetContractType<T>() where T : class
    {
        return typeof(T);
    }
    ...
    private void ValidateContract(Type contractType, string path = null)
    {
        ...
        var originalType = _types
            .Where(x => x.FullName is not null &&
                        x.FullName.Contains($"Chime.Modules.{module}", StringComparison.InvariantCultureIgnoreCase))
