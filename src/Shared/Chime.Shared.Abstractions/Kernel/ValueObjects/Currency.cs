...
public class Currency : IEquatable<Currency>
{
    ...
    {
        if (string.IsNullOrWhiteSpace(value) || value.Length != 3) throw new InvalidCurrencyException(value);

        value = value.ToUpperInvariant();
        if (!AllowedValues.Contains(value)) throw new UnsupportedCurrencyException(value);
